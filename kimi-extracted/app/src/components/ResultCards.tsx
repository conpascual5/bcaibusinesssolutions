...
// --- Legacy cards (still needed for Library compatibility) ---
export function TrendCard({ platform, icon, score, summary, tags }: {
  platform: string; icon: string; score: number; summary: string; tags: string[];
}) {
  return (
    <div className="bg-white rounded-2xl p-5 border border-gray-200">
      ...
      <div className="flex flex-wrap gap-2">
        {tags.map((tag, i) => (
          <span key={i} className="px-2 py-1 bg-gray-100 rounded-md text-xs text-gray-600">{tag}</span>
        ))}
      </div>
    </div>
  );
...
export function ThemeCard({ index, title, description }: { index: number; title: string; description: string }) {
  const colors = ["border-blue-300", "border-green-300", "border-purple-300", "border-orange-300", "border-pink-300"];
  return (
    <div className={`bg-white rounded-2xl p-5 border-2 ${colors[index % 5]} cursor-pointer hover:shadow-lg transition-all`}>
      <span className="text-3xl">{["📸","🎨","🏠","✨","🎯","🌿","🏙️","🎁","💼","🎬"][index % 10]}</span>
