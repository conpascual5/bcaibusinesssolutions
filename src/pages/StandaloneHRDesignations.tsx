...
type Designation = { id: string; title: string; code: string | null; description: string | null; };
...
export default function StandaloneHRDesignations() {
  ...
  const load = async () => {
    ...
    const { data } = await supabase.from("hr_designations").select("*").eq("business_id", businessOwnerId).order("title");
    if (data) setDesignations(data);
    setLoading(false);
  };
  useEffect(() => { load(); }, [businessOwnerId]);

  const resetForm = () => { setForm({ title: "", code: "", description: "" }); setEditing(null); setShowForm(false); };
  const openEdit = (d: Designation) => { setForm({ title: d.title, code: d.code || "", description: d.description || "" }); setEditing(d); setShowForm(true); };
  ...
  return (
    <HRLayout title="Designations" description="Define job titles and positions">
      {loading ? (
          ...
          {showForm && (
            <div className="bg-card rounded-2xl border border-border p-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div><label className="text-xs font-medium text-muted-foreground">Title *</label><input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} className="w-full mt-1 px-3 py-2 rounded-xl border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20" /></div>
