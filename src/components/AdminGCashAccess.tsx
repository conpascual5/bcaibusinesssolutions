...
import { Loader2, Check, X, Search, Smartphone, Shield, UserCheck, Pause, Play } from "lucide-react";
  ...
  useEffect(() => {
    (async () => {
      const [usersRes, accessRes, subsRes] = await Promise.all([
        ...
        supabase.from("subscriptions").select("*").eq("plan", "gcash").eq("status", "active"),
      ]);
  ...
  const toggleAccess = async (userId: string, hasAccess: boolean) => {
    ...
    try {
      if (hasAccess) {
        ...
        await supabase.from("subscriptions").update({ status: "cancelled", cancelled_at: new Date().toISOString() }).eq("user_id", userId).eq("plan", "gcash").eq("status", "active");
        ...
        const { data } = await supabase.from("subscriptions").select("*").eq("user_id", userId).eq("plan", "gcash").eq("status", "active").maybeSingle();
        if (data) {
          setSubscriptions((prev) => ({ ...prev, [userId]: data as SubscriptionRow }));
        }
        toast.success("GCash access granted. ₱199/month subscription created.");
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Grant or revoke GCash Cash In/Out Tracker access. ₱199/month — creates subscription with invoices.
          </p>
        </div>
      ...
      <div className="text-xs text-muted-foreground flex items-center gap-2">
        ...
        <span>Only non-admin users are shown. ₱199/month — Suspend stops access but keeps subscription record.</span>
      </div>
    </div>
  );
}
