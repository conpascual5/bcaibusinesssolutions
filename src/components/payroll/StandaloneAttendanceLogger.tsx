...
import type { Employee, PayrollPeriod, Schedule } from "@/pages/StandaloneHRPayroll";
...
export default function StandaloneAttendanceLogger({ employees, attendance, payrollPeriods, schedules, onRefresh, getEmployee }: Props) {
  ...
  const filteredAttendance = selectedPeriod
    ? attendance.filter((a: any) => a.date >= selectedPeriod.start_date && a.date <= selectedPeriod.end_date)
    : attendance;
