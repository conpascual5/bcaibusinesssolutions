...
const captionTemplates = {
  facebook: [
    ...
    "Sino ang gusto ng upgrade? I-try ang {product} at feel the difference! Mura pero premium quality. #musthave",
  ...
  tiktok: [
    ...
    "my honest review of {product} after 1 month of use spoiler alert: worth it! #review",
    "yung feeling na may {product} ka ang sarap sa feeling! #satisfying #aesthetic",
...
function generateVideoScripts(product: string, rng: () => number): VideoScript[] {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const visualPrompts: VisualPrompt[] = [
      { scene: "Hook Shot", prompt: allVisualPrompts[i * 3].replace(/{product}/g, product) },
      { scene: "Product Hero Shot", prompt: allVisualPrompts[i * 3 + 1].replace(/{product}/g, product) },
      { scene: "Lifestyle/CTA Shot", prompt: allVisualPrompts[i * 3 + 2].replace(/{product}/g, product) },
    ];
...
export function generateTargeting(product: string): TargetingResult {
  ...
  const productAdjKeywords = [
    `${product} enthusiasts`, `${product} reviews`, `${product} alternatives`,
    `Best ${product}`, `${product} deals`,
  ];
